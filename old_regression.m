...
%
% Output Arguments
% SSE_final: the SSE value of the model function
% SST: the SST value of the price data
%
...
price_measured = data(:,2); % separates the price from the data
...
SST = sum((price_measured - mean(price_measured)).^2); %calculate the SST value of the price data measured
%% Calculate the linearized function for the data 
...
SSE_final = min(SSE_value); %find the lowest SSE value
choose = find(SSE_value == SSE_final); %choose the corresponding model
...
title({'The Ezyme USD Price Per Pound of each';'Michaelis Constant from 157-350'})
function_output = sprintf("Model function: %.3f * Km ^ (%.3f)",b2,m2);
xlabel('Michaelis Constant (uM)'); 
ylabel('Price (USD($)/lb)')
...
fprintf("The chosen model is Power funciton: %.3f * Km ^ (%.3f)",b2,m2); 
