...
% ENGR 132 
% Program Description 
% This function will use the MOVING AVERAGE method in order to smooth
...
%% ____________________
%% CALCULATIONS
smoothPasses = 10; % the number of iterations the smoothing algorithm should use
...
for productNum = 1:10 % cycle between all of the 10 products
    ...
    for index = 1:smoothPasses:(length(P) - smoothPasses) % cycle through each [P] value
        % take average of segments of [P] whith width of smoothPasses
        sumPvals = sum(P(index:(index + smoothPasses))); 
        avgPvals = sumPvals / (smoothPasses + 1);
        
        % take average of segments of time whith width of smoothPasses
        sumTimes = sum(time(index:(index + smoothPasses)));
        avgTimes = sumTimes / (smoothPasses + 1);
        
        smoothedPVals = [smoothedPVals,avgPvals]; % add avg [P] to final array
        smoothedTimes = [smoothedTimes,avgTimes];% add avg time to final array
    end
    
    smoothedTimes = smoothedTimes(1:3);
    smoothedPVals = smoothedPVals(1:3);
    
    v0 = [v0, smoothedTimes(:) \ smoothedPVals(:)]; %find the first slope and add that to the v0 array
end
